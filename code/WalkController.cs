...
{
    [Library]
    public partial class DWWalkController : BasePlayerController
    {
		...
		//public SandboxPlayer.Controller.Player Player => Pawn as SandboxPlayer.Controller.Player;
		public Duck Duck;
        public Unstuck Unstuck;


        public DWWalkController()
        {
            Duck = new Duck(this);
        ...
        public override void Simulate()
        {
            ...
            //Rot = Rotation.LookAt( ViewAngles.ToRotation().Forward.WithZ( 0 ), Vector3.Up );
            ...
            */
            ...
            WishVelocity *= ViewAngles.ToRotation().Angles().WithPitch(0).ToRotation();
		...
		public override TraceResult TraceBBox( Vector3 start, Vector3 end, Vector3 mins, Vector3 maxs, float liftFeet = 0.0f )
		{
			...
			var tr = Trace.Ray( start + TraceOffset, end + TraceOffset )
				.Size( mins, maxs )
				.WithAnyTags( DWTags.Solid, "playerclip", "passbullets", "player" )
				.WithoutTags( "demat" )
				.WithoutTags( "opendoor" )
				.Ignore( Pawn );
			
			if ( Pawn.Tags.Has( DWTags.InPortal ) || Client.IsInAPortal() )
			{
				tr = tr.WithoutTags( DWTags.PortaledSurface )
						.WithAnyTags( DWTags.PortalGround );
				//if ( (Client.GetPlayer()as SandboxPlayer).Teleported < 0.4f )
				//	tr = tr.WithoutTags( DWTags.TopPortalGround );
			}
			var traceResult = tr.Run();
			traceResult.EndPosition -= TraceOffset;
		...
		public virtual void StepMove()
        {
			...
			mover.Trace = mover.Trace.WithoutTags( "demat" );
			mover.Trace = mover.Trace.WithoutTags( "opendoor" );
			if ( Pawn.Tags.Has( DWTags.InPortal ) || Client.IsInAPortal() )
			{
				///if ( (Client.GetPlayer() as SandboxPlayer).Teleported < 0.1f )
					mover.Trace = mover.Trace.WithoutTags( DWTags.TopPortalGround );
				mover.Trace = mover.Trace.WithoutTags( DWTags.PortaledSurface );
        ...
        public virtual void Move()
        {
			...
			if ( Pawn.Tags.Has( DWTags.InPortal ) || Client.IsInAPortal() )
			{
				mover.Trace = mover.Trace.WithoutTags( DWTags.PortaledSurface ).WithAnyTags( DWTags.PortalGround );
				///if (( Client.GetPlayer() as SandboxPlayer ).Teleported < 0.1f )
					mover.Trace = mover.Trace.WithoutTags( DWTags.TopPortalGround );
        ...
        public virtual void CheckLadder()
        {
            ...
            wishvel *= ViewAngles.ToRotation().Angles().WithPitch(0).ToRotation();
